...
(defn get-areas
  "Areas de trabajo todas"
  []
  (Query db [areas-sql]))
;; End areas_trabajo

(defn get-docentes-nombre [docentes-id]
  (let [docente (:docente (first (Query db ["SELECT CONCAT(nombre,' ',paterno,' ',materno) as docente FROM docentes WHERE id = ?" docentes-id])))]
    docente))

;; Start get-docentes
(def get-docentes-sql
  "
  SELECT
  id AS value,
...
(defn get-docentes
  "Gets all docentes from database :ex: (get-docentes)"
  []
  (Query db [get-docentes-sql]))
;; End get-dosentes
